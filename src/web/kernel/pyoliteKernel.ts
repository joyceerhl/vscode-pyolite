...
export class PyoliteKernel extends BaseKernel implements IKernel {
   ...
   */
  constructor(options: PyoliteKernel.IOptions, context: vscode.ExtensionContext) {
    super(options);

    this._worker = new Worker(vscode.Uri.joinPath(context.extensionUri, 'pyodide.js').toString());
  ...
  async executeRequest(
    ...
    console.log('Sending request message to worker', content);
  ...
  private async _sendRequestMessageToWorker(type: string, data: any): Promise<any> {
    ...
    this._worker.postMessage({ type, data, parent: {
      buffers: [],
      channel: 'shell', 
      header: {
        msg_id: uuid(),
